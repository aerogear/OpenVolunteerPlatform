...
Open Source project that builds mobile application that organize distribution of food and mental health for people
affected by events that require strict quarantine at home. In many cases people are isolated and strugling to find help without phisical contact.

=== How this works

Volounteers distributing leaflets in communities that can be scanned using phone (google forms) to answer couple basic questions about their current situation and any personal or medical neeeds. The same leaflet can be used to gather posible volounteers. 
Form is dynamically processed using Data aqusition techniques to match people that might need help, 
seeking someone to talk or need critical resources to survive. 
 
AtHomeCrisisCommunity is an Mobile Application (PWA) that can be used by volounteers to collaborate work and connect
